...
import {
  ...
  testResultMeasurementLifeLabsUnlinkedCaseFixture,
...
describe('LifeLabs Integration: parse and sync lab xml file: Test', () => {
  ...
  beforeAll(async () => {
    ...
    await specimenSeed.create(specimenExternalSameCollectedOnForLabIntegrationFixture)

    await testResultSeed.create(testResultForLifeLabsToBeWaitingForCompletionFixture)
    await testResultSeed.create(testResultForLifeLabsUnlinkedCaseFixture)

    await testResultMeasurementSeed.create(testResultMeasurementForLabIntegrationFixture)
    await testResultMeasurementSeed.create(testResultMeasurementLifeLabsUnlinkedCaseFixture)

    await labSyncRawDataSeed.create(labSyncRawDataFromLifeLabsPendingFixture)
  })

  test('should parse & sync raw data: multiple results found same specimen collected date => Unlinked & NotReceived', async () => {
    ...
    expect(testResult.status).toBe(testResultForLifeLabsToBeWaitingForCompletionFixture.status)
  ...
  afterAll(async () => {
    ...
    await testResultMeasurementSeed.removeByIds([
      testResultMeasurementForLabIntegrationFixture.id,
      testResultMeasurementLifeLabsUnlinkedCaseFixture.id,
    ])
    await testResultSeed.removeByIds([
      testResultForLifeLabsToBeWaitingForCompletionFixture.id,
      testResultForLifeLabsUnlinkedCaseFixture.id,
    ])

    await specimenSeed.removeByIds([
      specimenExternalForLabIntegrationFixture.id,
      specimenExternalSameCollectedOnForLabIntegrationFixture.id,
    ])
