...
import {
  ...
  testResultMeasurementTwoForLinkedAndStatusForLabIntegrationFixture,
...
describe('LifeLabs Integration: parse and sync lab xml file: Test', () => {
  ...
  beforeAll(async () => {
    ...
    await testTypeSeed.create(testTypeWithoutIntegrationCodeForLabIntegrationFixture)
  ...
  test('should parse & sync raw data: Observation Request - Linked, Test Result - NotReceived', async () => {
    const file = await fs.readFile(__dirname + '/lifelabs/lifelabs-one-obr-and-two-obx.xml')
    ...
    expect(testResult.status).toBe(TestResultStatus.NotReceived)

    const measurements = await testResultMeasurementSeed.findByTestResultId(testResult.id)

    const someIsNotFilled = measurements.some((item) => !item.result)

    expect(someIsNotFilled).toBeTruthy()
  ...
  afterAll(async () => {
    ...
    await testResultMeasurementSeed.removeByIds([
      ...
      testResultMeasurementTwoForLinkedAndStatusForLabIntegrationFixture.id,
    ])
    ...
    await testTypeSeed.removeByIds([
      ...
      testTypeWithoutIntegrationCodeForLabIntegrationFixture.id,
